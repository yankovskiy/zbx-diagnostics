...
# Zabbix: дефолты параметров — src/zabbix_server/server.c (а также
# src/libs/zbxconf/cfg.c и src/libs/zbxaudit/audit_log.c), правила разбора
# конфига — src/libs/zbxconf/cfg.c, соединения с БД — вызовы DBconnect()
# в соответствующих процессах src/zabbix_server/.
...
# всегда). Поведение по исходникам: zbx_tfc_init() и zbx_vc_init() не выделяют
# память при размере 0, zbx_vmware_init() вызывается только при
...
def effective_values(params, warnings):
...
def report_required(params):
    ...
    print()
    if missing:
        print(f"  Не задано {len(missing)} из {len(REQUIRED_PARAMS)}: {', '.join(missing)}")
    else:
        print(f"  Все обязательные параметры заданы ({len(REQUIRED_PARAMS)}).")
    print()
    return missing
...
def report_db_connections(params, eff):
    print("=" * 72)
    print("3. Оценка количества соединений с базой данных")
    print("=" * 72)
    print("  Каждый перечисленный процесс открывает 1 соединение (вызов DBconnect()).")

    entries = []
    for name, param, source in DB_CONN_PROCESSES:
        if param is None:
            entries.append((name, None, "—", 1, source))
        else:
            instances = eff[param]
            entries.append((name, param, f"{param}={instances}", instances, source))
    for name, param, source in DB_CONN_CONDITIONAL:
    ...
    rows = []
    for name, param in DB_NO_CONNECTIONS:
        if param is None:
            rows.append((name, "—", 1))
        else:
...
def main():
    ...
    try:
        params, warnings = parse_config(args.config)
    except OSError as e:
        print(f"Ошибка: не удалось прочитать конфиг: {e}", file=sys.stderr)
        return 1

    eff = effective_values(params, warnings)
    ...
    missing = report_required(params)
    report_memory(params, eff)
    report_db_connections(params, eff)
